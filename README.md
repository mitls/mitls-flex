...
it includes command-line tools that use this library to illustate
...
First, please check you have all prerequisite, 3rd party software, as
...
type checked.  Type checking requires F7 and Z3. Note that the latest
version of F7 we use is currently not released.
...
provides a machine-checked proof in EasyCrypt that the master KEM of
...
Diffie-Hellman ciphersuites additionally requires default parameters
to be loaded from file.  A sample default-dh.pem file with default
...
If the installation of the certificates work, you should be prompted
...
The FlexTLS library and the FlexApps console application is a tool,
...
The FlexApps project stores the scenarii; Application.fs implements
the command line interface and is the file where the calls to those
...
solution is built in Debug mode. It is the perfect location to proceed
with your own scenario experimentations.
...
Several scenarii are available including:
- fh :
  "full handshake", it will run a standard, correct full TLS handshake

- smacktls :
  Several wrong traces, used to detect implementation issues
