...
(* Stub for supporting more point format options *)
let curve_name (p:ecdhparams) : ec_curve =
    match TLSConstants.parseEcCurve (curve_id p) with
    | Error(_) -> failwith "Incorrect Elliptic Curve"
    | Correct(curve) -> curve
