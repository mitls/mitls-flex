...
type Handshake_full_DHE =
    ...
    /// CLIENT - Run DHE Handshake with server authentication only
    ...
    /// CLIENT - Run DHE Handshake with mutual server and client authentication
    ...
    /// SERVER - Run DHE Handshake with server side authentication only *)
    ...
    static member server (listening_address:string, chain:Cert.chain, ?port:int, ?timeout:int) : state =
        ...
        if not (List.exists (fun cs -> cs = TLS_DHE_RSA_WITH_AES_128_GCM_SHA256) (FlexClientHello.getCiphersuites fch)) then
        ...
        let fsh = { FlexConstants.nullFServerHello with
            ciphersuite = Some(TLS_DHE_RSA_WITH_AES_128_GCM_SHA256)} in
    ...
    /// SERVER - Run DHE Handshake with mutual DHE with mutual server and client authentication
    ...
    static member server_with_client_auth (listening_address:string, chain:Cert.chain, ?port:int, ?timeout:int) : state =
        ...
        if ( not (List.exists (fun cs -> cs = TLS_DHE_RSA_WITH_AES_128_GCM_SHA256) (FlexClientHello.getCiphersuites fch)) ) || (FlexClientHello.getPV fch) <> TLS_1p2 then
            failwith (perror __SOURCE_FILE__ __LINE__ "No suitable ciphersuite given")
        else
